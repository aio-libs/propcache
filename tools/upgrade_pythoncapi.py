...
INDENTATION_REGEX = rf"^{SPACE_REGEX}*"
...
def get_member_regex(member):
    ...
    regex = r"(?<!Py_CLEAR\()" + get_member_regex_str(member) + r"(?!\s*=\s*)"
    return re.compile(regex)


def assign_regex_str(var, expr):
    # Match "var = expr;".
    return rf"{var} *= *{expr}\s*;"
...
class Py_TYPE(Operation):
    NAME = "Py_TYPE"
    REPLACE = ((get_member_regex("ob_type"), r"Py_TYPE(\1)"),)
    # Py_TYPE() was added to Python 2.6.


class Py_SIZE(Operation):
    NAME = "Py_SIZE"
    REPLACE = ((get_member_regex("ob_size"), r"Py_SIZE(\1)"),)
    # Py_SIZE() was added to Python 2.6.


class Py_REFCNT(Operation):
    NAME = "Py_REFCNT"
    REPLACE = ((get_member_regex("ob_refcnt"), r"Py_REFCNT(\1)"),)
...
class PyFrame_GetBack(Operation):
    ...
    REPLACE = ((get_member_regex("f_back"), r"_PyFrame_GetBackBorrow(\1)"),)
...
class PyFrame_GetCode(Operation):
    ...
    REPLACE = ((get_member_regex("f_code"), r"_PyFrame_GetCodeBorrow(\1)"),)
...
class PyThreadState_GetInterpreter(Operation):
    ...
    REPLACE = ((get_member_regex("interp"), r"PyThreadState_GetInterpreter(\1)"),)
...
class PyThreadState_GetFrame(Operation):
    ...
    REPLACE = ((get_member_regex("frame"), r"_PyThreadState_GetFrameBorrow(\1)"),)
...
class Py_NewRef(Operation):
    ...
    REPLACE = (
        ...
        (
            re.compile(
                rf"({INDENTATION_REGEX})"
                + rf"Py_(X?)INCREF\(({EXPR_REGEX})\)\s*;"
                + same_indentation(r"\1")
                + rf"return {OPT_CAST_REGEX}\3;",
                re.MULTILINE,
            ),
        ...
        (
            re.compile(
                rf"({INDENTATION_REGEX})"
                + rf"Py_(X?)INCREF\(({EXPR_REGEX})\);"
                + same_indentation(r"\1")
                + assign_regex_str(rf"({EXPR_REGEX})", rf"{OPT_CAST_REGEX}\3"),
                re.MULTILINE,
            ),
            r"\1\4 = Py_\2NewRef(\3);",
        ),
        ...
        (
            re.compile(
                rf"Py_(X?)INCREF\(({EXPR_REGEX})\);"
                + rf"{SPACE_REGEX}*"
                + assign_regex_str(rf"({EXPR_REGEX})", rf"{OPT_CAST_REGEX}\2")
            ),
            r"\3 = Py_\1NewRef(\2);",
        ),
        ...
        (
            re.compile(
                rf"({INDENTATION_REGEX})"
                + assign_regex_str(
                    rf"({EXPR_REGEX})", rf"{OPT_CAST_REGEX}({EXPR_REGEX})"
                )
                + same_indentation(r"\1")
                + r"Py_(X?)INCREF\((?:\2|\3)\);",
                re.MULTILINE,
        ...
        (
            re.compile(
                assign_regex_str(rf"({EXPR_REGEX})", rf"{OPT_CAST_REGEX}({EXPR_REGEX})")
                + rf"{SPACE_REGEX}*"
                + r"Py_(X?)INCREF\((?:\1|\2)\);"
            ),
            r"\1 = Py_\3NewRef(\2);",
        ...
        (
            re.compile(
                ...
                # "type* var = expr;"
                + assign_regex_str(
                    rf"({TYPE_PTR_REGEX} *)({EXPR_REGEX})",
                    rf"({OPT_CAST_REGEX})({EXPR_REGEX})",
                )
                + same_indentation(r"\1")
                # "Py_INCREF(var);"
                + r"Py_(X?)INCREF\((?:\3|\5)\);",
        ...
        (
            re.compile(
                rf"({INDENTATION_REGEX})"
                # "Py_INCREF(var);"
                + rf"Py_(X?)INCREF\(({EXPR_REGEX})\);" + same_indentation(r"\1")
                # "type* var = expr;"
                + assign_regex_str(
                    rf"({TYPE_PTR_REGEX} *{EXPR_REGEX})", rf"({OPT_CAST_REGEX})\3"
                ),
                re.MULTILINE,
            ),
...
SETREF_VALUE = rf"{OPT_CAST_REGEX}(?:{EXPR_REGEX}|Py_X?NewRef\({EXPR_REGEX}\))"
    ...
    REPLACE = (
        ...
        (
            re.compile(
                ...
                + rf"Py_CLEAR\(({EXPR_REGEX})\) *;"
                + same_indentation(r"\1")
                + assign_regex_str(r"\2", rf"({SETREF_VALUE})"),
                re.MULTILINE,
            ),
            r"\1Py_XSETREF(\2, \3);",
        ),
        ...
        (
            re.compile(
                ...
                + rf"Py_(X?)DECREF\(({EXPR_REGEX})\) *;"
                + same_indentation(r"\1")
                + assign_regex_str(r"\3", rf"({SETREF_VALUE})"),
                re.MULTILINE,
            ),
            r"\1Py_\2SETREF(\3, \4);",
        ),
        ...
        (
            re.compile(
                rf"({INDENTATION_REGEX})"
                + rf"(?:{ID_REGEX} *\* *)?({ID_REGEX}) *= *{OPT_CAST_REGEX}({EXPR_REGEX}) *;"
                + same_indentation(r"\1")
                + assign_regex_str(r"\3", rf"({SETREF_VALUE})")
                + same_indentation(r"\1")
                + rf"Py_(X?)DECREF\(\2\) *;",
...
class Py_Is(Operation):
    ...
    for name in ("None", "True", "False"):
        REPLACE.extend(
            (
                (re.compile(rf"{expr} == Py_{name}\b"), rf"Py_Is{name}(\1)"),
                (re.compile(rf"{expr} != Py_{name}\b"), rf"!Py_Is{name}(\1)"),
            )
        )
...
class Patcher:
    ...
    def usage(parser):
        ...
        for operation in sorted(
            OPERATIONS, key=lambda operation: operation.NAME.lower()
        ):
            print(f"- {operation.NAME}")
        print()
        print(
            "If a directory is passed, search for .c and .h files " "in subdirectories."
        )
    ...
    def _parse_options(self, args):
        ...
        parser.add_argument(metavar="file_or_directory", dest="paths", nargs="*")
    ...
    def main(self):
        ...
        if self.pythoncapi_compat_added and not self.args.quiet:
            self.log()
            self.log(
                f"{INCLUDE_PYTHONCAPI_COMPAT} added: you may have "
